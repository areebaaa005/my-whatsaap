...
export default function ChatWindow({ activeContact, messagesEndRef, initials, startCall, setActiveId }) {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: (showInfo && !isMobile) ? "1px solid #d1d7db" : "none" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 8 : 12 }}>
            ...
            {isMobile && (
              <span 
                ...
                style={{ cursor: "pointer", fontSize: "20px", paddingRight: "5px", color: "#54656f" }}
              >
                ←
              </span>
            )}
            ...
            <div 
              ...
              style={{ 
                ...
                fontWeight: "600", fontSize: "16px", overflow: "hidden", cursor: "pointer" 
              }}
            >
              {activeContact.img ? (
            ...
            <div onClick={() => setShowInfo(true)} style={{ cursor: "pointer" }}>
              <div style={{ color: "#111b21", fontWeight: 400, fontSize: isMobile ? "15px" : "16px" }}>{activeContact.name}</div>
              <div style={{ color: "#667781", fontSize: "12px" }}>{activeContact.status}</div>
            </div>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "12px" : "22px", color: "#54656f" }}>
            ...
            {/* Call button text hidden on mobile for space */}
            <div 
              style={{ display: "flex", alignItems: "center", gap: "8px", padding: isMobile ? "6px" : "6px 14px", borderRadius: "20px", border: "1px solid #d1d7db", background: "#ffffff", cursor: "pointer" }}
              onClick={() => startCall?.('audio')}
            >
              <span>📞</span> {!isMobile && <span style={{ fontSize: "13.5px", color: "#111b21", fontWeight: 500 }}>Call</span>}
            </div>
        ...
        {isMsgSearching && (
          <div style={{ background: "#ffffff", padding: "8px 16px", borderBottom: "1px solid #e9edef", display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ flex: 1, background: "#f0f2f5", borderRadius: "8px", display: "flex", alignItems: "center", padding: "0px 12px", height: "35px" }}>
              ...
              <input autoFocus type="text" placeholder="Search..." value={msgSearchQuery} onChange={(e) => setMsgSearchQuery(e.target.value)} style={{ background: "transparent", border: "none", outline: "none", color: "#3b4a54", padding: "6px 10px", width: "100%", fontSize: "14px" }} />
            </div>
            <span style={{ cursor: "pointer", color: "#00a884", fontSize: "13px" }} onClick={() => setIsMsgSearching(false)}>Cancel</span>
          </div>
        )}

        {/* Messages Container */}
        <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "15px 10px" : "20px 7%", position: "relative" }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            {(msgSearchQuery ? filteredMsgs : activeContact.msgs).map((msg) => (
              <div key={msg.id} style={{ display: "flex", justifyContent: msg.from === "me" ? "flex-end" : "flex-start", marginBottom: 4 }}>
                {msg.type === "call_log" ? (
                   <div style={{ background: "#ffffff", padding: "8px", borderRadius: "10px", width: "240px", display: "flex", alignItems: "center", gap: "10px", boxShadow: "0 1px 1px rgba(0,0,0,0.08)" }}>
                    <div style={{ width: "30px", height: "30px", borderRadius: "50%", background: "#f0f2f5", display: "flex", alignItems: "center", justifyContent: "center" }}><span>{msg.callType === "video" ? "📹" : "📞"}</span></div>
                    <div>
                      <div style={{ fontWeight: "500", fontSize: "13px" }}>{msg.callType === "video" ? "Video call" : "Voice call"}</div>
                      <div style={{ fontSize: "11px", color: "#667781" }}>No answer</div>
                    </div>
                  </div>
                ) : (
                  <div style={{ 
                    ...
                    {msg.type === "image" ? (
                      ...
                      <audio src={msg.url} controls style={{ height: "32px", width: isMobile ? "180px" : "210px" }} />
                    ) : (
                      <span style={{ fontSize: "14.2px", color: "#111b21" }}>{msg.text}</span>
                    )}
      ...
      {showInfo && (
        <div style={{ 
          ...
          <div style={{ background: "#ffffff", padding: "16px 20px", display: "flex", alignItems: "center", gap: "25px", height: "60px" }}>
            <span onClick={() => setShowInfo(false)} style={{ cursor: "pointer", fontSize: "18px" }}>✕</span>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "28px 16px", background: "#ffffff", marginBottom: "10px" }}>
              ...
              <h2 style={{ color: "#111b21", fontSize: "20px", fontWeight: 400 }}>{activeContact.name}</h2>
              <p style={{ color: "#667781", fontSize: "14px" }}>+92 323 9704313</p>
            </div>
            
            {/* ... baaki content (About section etc) ... */}
            <div style={{ padding: "20px", background: "#ffffff" }}>
              <p style={{ color: "#667781", fontSize: "14px", marginBottom: "12px" }}>About</p>
              <p style={{ fontSize: "15px", color: "#111b21" }}>{activeContact.about || "Hey there! I am using WhatsApp."}</p>
            </div>
          </div>
        </div>
      )}
